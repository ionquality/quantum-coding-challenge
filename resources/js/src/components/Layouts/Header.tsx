...
import { getUser, logout, isAuthenticated } from '../../utils/auth'; // Import functions from auth.ts
...
const Header = () => {
    ...
    const [user, setUser] = useState<any>(null); // State to store the logged-in user's details
    const isRtl = useSelector((state: any) => state.themeConfig.rtlClass) === 'rtl';

    // Fetch the logged-in user's details
    useEffect(() => {
        const fetchUser = async () => {
            const userData = await getUser(); // Use getUser from auth.ts
            setUser(userData);
        };
    ...
    const handleLogout = () => {
        logout(); // Use logout function from auth.ts
        window.location.href = '/auth/cover-login'; // Redirect to login page
    };
                ...
                <div className="relative bg-white flex w-full items-center px-5 py-2.5 dark:bg-black">
                    <div className="horizontal-logo flex lg:hidden justify-between items-center ltr:mr-2 rtl:ml-2">
                        <Link to="/" className="main-logo flex items-center shrink-0">
                            <img className="w-8 ltr:-ml-1 rtl:-mr-1 inline" src="https://simpltrakstorage.nyc3.digitaloceanspaces.com/simpletrak4/image-assets/Quantum-ePay-Full-Color-Light.png" alt="logo" />
                            <span className="text-2xl ltr:ml-1.5 rtl:mr-1.5  font-semibold  align-middle hidden md:inline dark:text-white-light transition-all duration-300">Quantum</span>
                        </Link>
                        <button
                            type="button"
                                ...
                                button={
                                    user?.avatar ? (
                                        ...
                                        <div className="w-9 h-9 rounded-full bg-gray-300 flex items-center justify-center text-white font-bold">
                                            {user?.name ? getUserInitials(user.name) : 'NA'}
                                        </div>
                                    )
                                }
                            >
                                    ...
                                    <li>
                                        <div className="flex items-center px-4 py-4">
                                            ...
                                            <div className="ltr:pl-4 rtl:pr-4 truncate">
                                                <h4 className="text-base">{user?.name || 'Guest'}</h4>
                                                <span className="text-sm text-gray-500">{user?.email || 'guest@example.com'}</span>
                                            </div>
                                        </div>
                                    </li>
